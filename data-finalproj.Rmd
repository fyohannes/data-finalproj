...
Second graph for Shiny Demo, FACET GRID FOR JOBS
```{r}
un_graph %>%
...
x %>%
  ggplot(aes(x=workload,y=overall)) +
   #Plotting Overall scores in relation to workload
  geom_point(color="green") +
  #Changing the alpha transparency of the points
  geom_smooth(se = FALSE, method = "lm") +
  #Adding a best fit line
  facet_grid(. ~ divisional_distribution) +
   labs(title = "Change in job sectors",
              x = "Years",
              y= "% of employed population")
...
```{r}
world_bank %>%
  filter(str_detect(Indicator.Name,fixed("School enrollment, primary, male"))) %>%
    ggplot(aes(x=Years_new,y=value)) +geom_point() +
  geom_smooth(se = FALSE, method = "lm")
```
